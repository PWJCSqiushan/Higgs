...
官方 QQ Bot 使用单独私有绑定的 owner OpenID，但会解析到同一个内部 owner principal。官方主人 C2C 支持只读状态/记忆/提醒/计划命令，以及经过事件级持久领取的低风险变更；同一平台事件不会重复执行，结果未知时也不会自动重试。`whitelist` 和 `natural` 仍只管理个人 QQ 数字身份，不允许从官方 OpenID 通道修改。官方群中的主人命令一律拒绝。
...
`disable` 只暂停普通回复；两条 transport 的监听、消息日志、提醒调度和主人命令仍保持运行，因此可以随时用 `enable` 恢复。`status` 匿名分列个人 QQ/OneBot 与官方 Gateway，不回显账号或 OpenID。`risk` 显示 24 小时发送、失败、限流、半小时峰值、疑似机器人来源和被踢次数，不显示聊天正文。
...
`rate 2 6` 表示普通单会话每分钟最多生成 2 次回复、所有会话合计每分钟最多 6 次。系统还有小时、每日硬上限和非主人会话熔断；聊天命令不能绕过这些保护。`debounce` 范围为 0.5–10 秒，用于合并同一会话、同一个人的连续短句；热更新同时作用于 OneBot 内存合并器与官方 durable quiet-window，不需要重启 transport。
