...
def test_all_runtime_databases_can_be_verified_and_restored(tmp_path: Path) -> None:
    ...
    assert len(BackupManager.DATABASE_NAMES) == 13
    ...
    assert "tool_audit.sqlite" in BackupManager.DATABASE_NAMES
    assert "official_processing.sqlite" in BackupManager.DATABASE_NAMES
