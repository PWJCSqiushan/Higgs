...
| 独立人格 | Persona 2.2 已在官方 owner C2C 上线；普通用户和官方群已有独立默认关闭门，聊天内容不能修改 `self_core` 或主人关系 |
| 模型接入 | OpenAI-compatible API；已验证智谱 GLM；支持 `live`、`draft`、`off` 三种模式 |
| 回复治理 | 连续短消息合并、纯文本输出、敏感内容过滤、通道回执校验、会话与全局限频；官方群处理和发送边界均持久化 |
| 长期记忆 | Memory V2.1 的观察、审核和混合召回；另有默认关闭的 Personal Memory V5，用于普通用户本人明确记住、双次佐证、精确纠正和自然遗忘 |
...
普通官方 C2C 与官方群使用 Bot 账户绑定的版本化白名单。每次捕获窗口和冻结版本都有独立
来源、前序版本与规范指纹；Agent 与 Sidecar 的版本、指纹或受众集合任一不一致都会失败
关闭。该代码存在不表示生产受众已经扩大，身份库迁移、白名单激活和真实验收仍是分开的
显式动作。
...
2. **后台整理**：默认每 15 分钟处理一批、每批最多 50 条。单条坏观察会单独标记为 `failed`，不会卡住整个队列。若单独批准启用 Personal Memory V5，普通用户明确的记住、纠正和遗忘会先走同一份幂等事务并得到简短确认，后台重放只负责收束观察，不会重复改写。
...
官方 QQ 的今日计划只允许显式绑定的主人 C2C。草案和只读查看可保持在受控模式；`live` 确认只有在 Agent/sidecar proactive 双门通过单独生产审批后才会创建节点提醒。提醒固定投递到确认时的通道、Bot account 与 owner 目标，不会在官方 Bot 与 NapCat 之间自动切换。
