...
发送使用唯一 occurrence key。服务器重启时会恢复未到期节点；QQ 离线时不发送，过期节点标为 `missed`，不会在很久以后补发失去意义的消息。
...
这三个数据库与既有七库一起进入一致性十库备份。API Key、OneBot token、NapCat 登录状态和地点正文不会写入备份清单或 Git 配置快照。
