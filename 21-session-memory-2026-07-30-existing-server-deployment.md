...
- 云端首次实聊暴露出私聊短消息未经过群聊防抖器的问题：同一人连续发送
  多个消息气泡会触发多次模型调用和逐条回复。为保护账号，发现后只暂停
  Higgs 回复容器，NapCat 与 QQ 登录保持在线。
- 提交 `5a1673d` 将防抖扩展到私聊：同一会话、同一发送者的连续消息按
  独立安静窗口合并，不同私聊发送者互不干扰；默认私聊窗口为 4 秒。
- QQ 主人命令 `/higgs debounce` 现在会同时更新群聊与私聊合并窗口，并
  将两项配置持久化。
- 云端已切换到不可变镜像 `higgs-agent:5a1673d`，单会话/全局回复上限
  收紧为每分钟 4/12 条。升级只重建 Higgs，未重启 NapCat。
...
- 私聊防抖：单元测试覆盖同一人三条合并及不同发送者隔离；完整测试
  122 项通过，Ruff 与格式检查通过。
- 资源：验收时 Higgs 约 29MB/384MB，NapCat 约 243MB/960MB。
- 原有 Nginx、FurColor、超星、宝塔服务在升级后均保持 active。
- 本机 Higgs 已停止；本机 SSH 隧道只用于访问服务器回环 WebUI。

## 待办

1. 主人大号快速连续发送三条短消息，确认约 4 秒后只形成一次回复；再发送
   `/higgs status` 验证主人硬绑定和命令通道。
